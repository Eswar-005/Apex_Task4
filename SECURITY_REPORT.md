...
*   **Vulnerability Description**: Any registered user could access post creation, modification, and deletion features by guessing routes or sending direct HTTP requests to administrative pages.
*   **Before**: No role check existed, and all authenticated accounts shared a single flat set of permissions.
*   **After**:
    - Added a `role` ENUM column (`admin`, `editor`, `user`) to the database schema.
...
To provide a programmatical proof of prepared statements security, a dedicated test script was built at [tests/sqli_proof.php](file:///c:/Users/Lalitha/apexPlanet_task4/tests/sqli_proof.php). It runs an SQL injection payload (`admin_jane' OR '1'='1`) against both a vulnerable concatenated query simulation and a secure prepared query.
...
Simulated Attack Input Payload: admin_jane' OR '1'='1

--- DEMO 1: Vulnerable Direct Concatenation ---
Query: SELECT id, username, role FROM users WHERE username = 'admin_jane' OR '1'='1'
Status: 🔴 BREACHED!
Records Leaked from Database:
  - [ID: 1] Username: admin_jane | Role: admin
  - [ID: 2] Username: editor_user | Role: editor
  - [ID: 3] Username: test_user | Role: user
